...
router.get('/dashboard', requireAuth, async (req, res) => {
  try {
    const { data: items, error } = await supabase
      .from('items').select('*').eq('user_id', req.session.userId)
    ...
    res.render('dashboard', {
      user:  { name: req.session.userName, plan: req.session.userPlan },
      items: [], stats: { total: 0, active_tags: 0, lost_items: 0 },
    });
  }
});
...
// ── GET /add-item ───────────────────────────────────────────
router.get('/add-item', requireAuth, (req, res) => res.render('add-item'));
...
router.post('/add-item', requireAuth, async (req, res) => {
  ...
  try {
    ...
    while (exists) {
      tagId = generateTagId();
      const { data } = await supabase.from('items').select('id').eq('tag_id', tagId).limit(1);
      exists = data && data.length > 0;
    }

    const qrUrl     = `${req.protocol}://${req.get('host')}/finder/${tagId}`;
    const qrDataUrl = await QRCode.toDataURL(qrUrl, {
      color: { dark: '#0d9e82', light: '#ffffff' }, width: 400, margin: 2,
    });

    const { error: insertErr } = await supabase.from('items').insert({
      user_id: req.session.userId, name: itemName, category,
      description: description || null, finder_message: finderMessage || null,
      tag_id: tagId, qr_image_path: qrDataUrl,
    });
    if (insertErr) throw insertErr;

    req.session.flash_success = `"${itemName}" registered with tag ${tagId}!`;
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>
    <script>window.location.href='/item/${tagId}';</script></body></html>`);
  } catch (err) {
    console.error('Add item error:', err.message);
    req.flashError('Could not register item: ' + err.message);
...
router.get('/item/:tagId', requireAuth, async (req, res) => {
  try {
    const { data: items, error } = await supabase
      .from('items').select('*')
      .eq('tag_id', req.params.tagId).eq('user_id', req.session.userId).limit(1);
    if (error) throw error;
    ...
    const { data: scanData } = await supabase
      .from('scans').select('scanned_at, latitude, longitude, location_name')
    ...
    req.flashError('Could not load item: ' + err.message);
...
router.post('/item/:tagId/toggle-lost', requireAuth, async (req, res) => {
  try {
    const { data: items } = await supabase
      .from('items').select('id, lost_mode')
      .eq('tag_id', req.params.tagId).eq('user_id', req.session.userId).limit(1);
    if (!items || !items[0]) return res.json({ ok: false, error: 'Not found' });

    const newLostMode = !items[0].lost_mode;
    const newStatus   = newLostMode ? 'lost' : 'active';
    const { error } = await supabase.from('items')
      .update({ lost_mode: newLostMode, status: newStatus }).eq('id', items[0].id);
    if (error) throw error;
...
router.post('/item/:tagId/delete', requireAuth, async (req, res) => {
  try {
    const { error } = await supabase.from('items')
      .delete().eq('tag_id', req.params.tagId).eq('user_id', req.session.userId);
    if (error) throw error;
    req.session.flash_success = 'Item deleted.';
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>
    <script>window.location.href='/my-items';</script></body></html>`);
  } catch (err) {
    req.session.flash_error = 'Could not delete item.';
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>
    <script>window.location.href='/my-items';</script></body></html>`);
  }
});
